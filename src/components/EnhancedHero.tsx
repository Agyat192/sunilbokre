...
const EnhancedHero = () => {
  ...
  // Track mouse position for background effects and detect mobile
  useEffect(() => {
    const checkMobile = () => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0">
        {/* Dynamic gradient orbs that follow mouse - reduced on mobile */}
        {!isMobile && (
          <motion.div
            className="absolute w-96 h-96 rounded-full blur-[120px]"
            style={{
              background: `radial-gradient(circle at ${mousePosition.x}% ${mousePosition.y}%, hsl(var(--primary) / 0.1), transparent 50%)`,
        ...
        {!isMobile && (
          <motion.div
            className="absolute w-96 h-96 rounded-full blur-[120px]"
            style={{
              background: `radial-gradient(circle at ${100 - mousePosition.x}% ${100 - mousePosition.y}%, hsl(0 0% 20% / 0.08), transparent 50%)`,
        ...
        {[...Array(isMobile ? 2 : 4)].map((_, i) => (
          <motion.div
            key={i}
            className="absolute w-6 h-6 sm:w-8 sm:h-8 border border-primary/20"
            style={{
              left: `${15 + (i * 20)}%`,
              top: `${20 + (i * 15)}%`,
            }}
            animate={{
              rotate: [0, 180, 360],
              scale: [1, 1.1, 1],
              x: [0, isMobile ? 10 : 20, -10, 0],
              y: [0, isMobile ? -10 : -20, 10, 0]
            }}
            transition={{
              duration: isMobile ? 15 : 10 + i * 2,
              repeat: Infinity,
              ease: 'linear'
            }}
          />
        ...
        {[...Array(isMobile ? 8 : 15)].map((_, i) => (
          <motion.div
            key={`particle-${i}`}
            className="absolute w-0.5 h-0.5 sm:w-1 sm:h-1 rounded-full bg-primary"
            style={{
